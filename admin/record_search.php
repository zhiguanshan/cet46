	...
	<body background="../img/body_bg.jpg">
		...
		<div class="right">
			...
			<?php
				$id=$_POST['CD'];
				if(strlen($id)==11){
					$sql = "SELECT * FROM sc where stu_id='$id'";
					$result = $db->query($sql);
					$rows_num=$result->num_rows;
				}
				else if(strlen($id)==6){
					$sql = "SELECT * FROM sc where course_id='$id'";
					$result = $db->query($sql);
					$rows_num=$result->num_rows;
				}
				else{
			...
			<table style="margin:40px auto;opacity: 0.8;" border="1px solid" width="1000px" align="center">
				<tr>
					<th width="10%" height="25px" align="center">学号</th>
					<th width="10%" height="25px" align="center">姓名</th>
					<th width="10%" height="25px" align="center">考试代码</th>
					<th width="20%" height="25px" align="center">考试名称</th>
					<th width="15%" height="25px" align="center">考试时间</th>
            ...
            ?>
            		<td>
            			<input type="button" name="button" id="button" value="删除" 
            				onclick="window.location.href='del_record.php?cid=<?=$row['course_id']?>'+'&&sid=<?=$row['stu_id']?>'"/>
            		</td>
            <?php
            	}
			?>
